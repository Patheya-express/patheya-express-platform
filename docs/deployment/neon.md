...
| Seed strategy (`prisma/seed.ts`) | Sixteen ordered seed modules (`prisma/seeds/01-*.seed.ts` through `16-*.seed.ts`, all active — no dead/unwired stubs) run via `tsx prisma/seed.ts` (`pnpm db:seed`), using a plain `new PrismaClient()` — reads `DATABASE_URL` the same way the app does, no environment-specific seed logic. Development-only baseline data, deterministic and idempotent (upsert/find-then-create by natural key), never real credentials or production data. |
